...
export class CoursesComponent implements OnInit {
  ...
  filteredCourses() {
    let finalArr = this.courses;

    if (this.type === '' || this.type === 'All') finalArr = this.courses;
    else
      finalArr = this.courses.filter(
        (course) => course.type.toLowerCase() === this.type.toLowerCase()
      );

    if (this.searchValue !== '' && this.searchValue.length >= 2) {
      finalArr = finalArr.filter((course) =>
  ...
  allCoursesLength: number = this.courses.length;
  premiumCoursesLength: number = this.courses.filter(
    (course) => course.type.toLowerCase() === 'premium'
  ).length;
  freeCoursesLength: number = this.courses.filter(
